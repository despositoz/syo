...
- ~~**Фокус-ловушка в `Sheet`**~~ — реализована; с P0.3.1 покрыта тестами
  (`Sheet.test.tsx` и e2e «a sheet keeps the keyboard inside it»).
...
- ~~**Один тест помечен `retry: 2`**~~ — retry снят в P0.3.1: причина была в
  самом тесте (клик приходил в узел, подменявшийся в тот же такт), теперь тест
  ждёт, пока страница устоится.

Обновлено после P0.3.1 — см. [`P0_3_1_ACCEPTANCE.md`](P0_3_1_ACCEPTANCE.md).
